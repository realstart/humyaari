   ...
   <div>
      <nav aria-label="breadcrumb">
         <ol class="breadcrumb">
            ...
            <li v-if="!success" class="breadcrumb-item"><a href="#">List of All Invoice</a></li>
                     ...
                     <table class="table table-hover general-table">
                        ...
                        <tbody>
                           <tr v-show="invoices.length" v-for="(invoice, index) in invoices" :key="invoice.id">
                              ...
                              <td> 
                                 <span  v-if="invoice.flag == 'GDS'" class="label label-info"> {{ invoice.flag }}</span> 
                                 <span  v-if="invoice.flag == 'Service'" class="label label-success"> {{ invoice.flag }}</span> 
                                 <span  v-if="invoice.flag == 'third party'" class="label label-warning"> {{ invoice.flag }}</span> 
                              </td>
   ...
   export default {
     ...
     methods: {
       ...
       loadInvoice(){
         ...
         this.$Progress.start()
        axios.get('/api/v1/invoice-list?page='+this.pagination.current_page) 
             .then(response => {
        ...
        SearchData(){
           ...
           this.$Progress.start()
             axios.get('/api/v1/search-invoice/'+this.queryField+'/'+this.query+'/?page='+this.pagination.current_page) 
             .then(response => {
